...
function extractToolNames(payload: string) {
  ...
  const tools =
    typeof message === 'object' && message !== null
      ? ((message as { result?: { tools?: Array<{ name?: unknown }> } }).result?.tools ?? [])
      : [];
...
void test('getServerConfig parses MCP_HTTP_ENABLED string flags explicitly', () => {
  assert.strictEqual(
    getServerConfig({ MCP_HTTP_ENABLED: 'false' } as NodeJS.ProcessEnv).MCP_HTTP_ENABLED,
    false,
  );
  assert.strictEqual(
    getServerConfig({ MCP_HTTP_ENABLED: '0' } as NodeJS.ProcessEnv).MCP_HTTP_ENABLED,
    false,
  );
  assert.strictEqual(
    getServerConfig({ MCP_HTTP_ENABLED: 'true' } as NodeJS.ProcessEnv).MCP_HTTP_ENABLED,
    true,
  );
});

void test(
  'OPTIONS /mcp returns CORS preflight headers for allowed origins',
  { concurrency: false },
  async () => {
...
void test(
  '/mcp returns 405 with explicit method handling for unsupported HTTP methods',
  { concurrency: false },
  async () => {
    ...
    try {
      for (const method of ['GET', 'DELETE'] as const) {
        ...
        try {
          ...
          assert.deepStrictEqual(response.json(), { message: 'Method Not Allowed' }, method);
        } finally {
          await server.close();
        }
      }
...
void test(
  'POST /mcp includes CORS headers for allowed origins',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({
      MCP_HTTP_ENABLED: 'true',
...
void test(
  'POST /mcp tools/list returns bootstrap tools when unauthenticated',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ MCP_HTTP_ENABLED: 'true' });
    const server = await buildServer();
...
void test(
  'POST /mcp tools/list hides root-only bootstrap tools for service keys',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ MCP_HTTP_ENABLED: 'true' });
    const server = await buildServer();
...
void test(
  'POST /mcp tools/list includes root-only bootstrap tools for root keys',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ MCP_HTTP_ENABLED: 'true' });
    const server = await buildServer();
...
void test(
  'POST /mcp rejects malformed authorization headers before tool registration',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ MCP_HTTP_ENABLED: 'true' });
    const server = await buildServer();
...
void test(
  'POST /mcp preserves CORS headers on auth failures for allowed origins',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({
      MCP_HTTP_ENABLED: 'true',
