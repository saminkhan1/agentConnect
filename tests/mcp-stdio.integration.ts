...
void test(
  'integration: stdio MCP entrypoint authenticates with AGENTINFRA_API_KEY and dispatches tools',
  { concurrency: false },
  async () => {
    ...
    try {
      ...
      assert.ok(toolNames.includes('agentinfra.payments.create_card_details_session'));
      ...
      assert.ok(listedAgents.agents.some((agent) => agent.id === createdAgent.agent.id));
    } finally {
