...
function createHookServer() {
  ...
  return {
    ...
    async listen() {
      ...
      assert.ok(address && typeof address !== 'string', 'expected hook server TCP address');
...
async function cleanupOrg(orgId: string) {
  await db.delete(webhookSubscriptions).where(eq(webhookSubscriptions.orgId, orgId));
...
void test(
  'integration: outbound webhooks deliver OpenClaw agent hook payloads with Bearer auth and signatures',
  { concurrency: false },
  async () => {
    ...
    try {
      ...
      assert.strictEqual(body.sessionKey, `agentconnect_evt_${writeResult.event.id}`);
      ...
      assert.strictEqual(envelope.subscription_id, createSubscriptionPayload.subscription.id);
...
void test(
  'integration: outbound webhooks reject hostnames that resolve to private addresses',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ NODE_ENV: 'test' });
    const server = await buildServer();
...
void test(
  'integration: outbound webhooks deliver OpenClaw wake payloads with x-openclaw-token and dedupe repeated events',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ NODE_ENV: 'test' });
    const hookServer = createHookServer();
...
void test(
  'integration: outbound webhook worker ignores stale deliveries after subscription deletion',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ NODE_ENV: 'test' });
    const hookServer = createHookServer();
    ...
    try {
      ...
      const delivery = await getDeliveryRow(createSubscriptionPayload.subscription.id);
      assert.ok(delivery, 'expected delivery row');

      await db
        .delete(webhookSubscriptions)
        .where(eq(webhookSubscriptions.id, createSubscriptionPayload.subscription.id));
...
void test(
  'integration: outbound webhooks honor Retry-After delta seconds for 429 responses',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ NODE_ENV: 'test' });
    const hookServer = createHookServer();
...
void test(
  'integration: outbound webhooks honor Retry-After HTTP dates for 429 responses',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ NODE_ENV: 'test' });
    const hookServer = createHookServer();
    ...
    try {
      ...
      assert.strictEqual(delivery.lastStatus, 'retry_scheduled');
      assert.strictEqual(delivery.attemptCount, 1);
      assert.strictEqual(delivery.nextAttemptAt.toISOString(), retryAt.toISOString());
    } finally {
      await cleanupOrg(orgId);
      await server.close();
      await hookServer.close();
...
void test(
  'integration: outbound webhooks retry 401, 429, and 5xx responses before succeeding',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ NODE_ENV: 'test' });
    const hookServer = createHookServer();
    ...
    try {
      ...
      hookServer.enqueueResponse({ statusCode: 401, body: { error: 'unauthorized' } });
      hookServer.enqueueResponse({ statusCode: 429, body: { error: 'rate_limited' } });
      hookServer.enqueueResponse({ statusCode: 503, body: { error: 'unavailable' } });
...
void test(
  'integration: outbound webhooks record stable 400 payload failures without retrying forever',
  { concurrency: false },
  async () => {
    const restoreEnv = setEnv({ NODE_ENV: 'test' });
    const hookServer = createHookServer();
    ...
    try {
      ...
      hookServer.enqueueResponse({ statusCode: 400, body: { error: 'bad_payload' } });
