...
void test("integration: issue_card persists resource/event and replays without reprovisioning", async () => {
	...
	server.resourceManager = new ResourceManager(
		new Map([["stripe", fakeStripeAdapter]]),
	);
	...
	try {
		...
		assert.strictEqual(
			firstPayload.event.eventType,
			EVENT_TYPES.PAYMENT_CARD_ISSUED,
		);
		assert.strictEqual(
			firstPayload.event.idempotencyKey,
			"issue-card-integration-idem",
		);
...
void test("integration: send_email uses the active inbox and replays idempotent sends without resending", async () => {
	...
	try {
		...
		assert.strictEqual(
			firstPayload.event.idempotencyKey,
			"send-email-integration-idem",
		);
		assert.strictEqual(performActionCalls.length, 1);
		assert.strictEqual(
			performActionCalls[0].providerRef,
			"active@agentmail.to",
		);
		...
		assert.strictEqual(
			storedEvents[0].resourceId,
			performActionCalls[0].resourceId,
		);
...
void test("integration: send_email recovers from provider success plus event write failure without resending", async () => {
	...
	wrappedWriter.writeEvent = async (
		input: Parameters<typeof originalWriter.writeEvent>[0],
	) => {
	...
	try {
		...
		assert.strictEqual(
			storedActionsAfterFailure[0].state,
			"provider_succeeded",
		);
		...
		assert.strictEqual(storedEvents[0].eventType, EVENT_TYPES.EMAIL_SENT);
		assert.strictEqual(
			storedEvents[0].idempotencyKey,
			"send-email-recover-idem",
		);
	} finally {
		await cleanupOrg(orgId);
		await server.close();
...
void test("integration: reply_email persists outbound action data and replays without re-dispatching", async () => {
	...
	const fakeAgentMailAdapter = {
		performAction: (
			...
			if (action === "get_message") {
				assert.strictEqual(
					payload.message_id,
					"msg_send_reply_integration_123",
				);
				return Promise.resolve({
					message_id: "msg_send_reply_integration_123",
					thread_id: "thread_reply_integration_123",
	...
	try {
		...
		assert.strictEqual(
			firstReplyPayload.event.eventType,
			EVENT_TYPES.EMAIL_SENT,
		);
		assert.strictEqual(
			firstReplyPayload.event.idempotencyKey,
			"reply-email-integration-idem",
		);
		...
		const getMessageCalls = performActionCalls.filter(
			(call) => call.action === "get_message",
		);
		const replyCalls = performActionCalls.filter(
			(call) => call.action === "reply_email",
		);
		assert.strictEqual(getMessageCalls.length, 1);
		assert.strictEqual(replyCalls.length, 1);
		assert.deepStrictEqual(replyCalls[0].payload.reply_recipients, [
		...
		const storedActions = await db
			...
			.from(outboundActions)
			.where(eq(outboundActions.orgId, orgId));
		const replyAction = storedActions.find(
			(action) => action.action === "reply_email",
		);
		assert.ok(replyAction);
		assert.strictEqual(replyAction.state, "completed");
		assert.deepStrictEqual(replyAction.requestData.reply_recipients, [
			"customer@example.com",
		]);
		...
		assert.strictEqual(
			replyEvent.data.in_reply_to_message_id,
			"msg_send_reply_integration_123",
		);
...
void test("integration: reply_email recovers from provider success plus event write failure without refetching or re-replying", async () => {
	...
	try {
		...
		assert.deepStrictEqual(
			replyActionAfterFailure.requestData.reply_recipients,
			["customer@example.com"],
		);
		assert.strictEqual(
			replyActionAfterFailure.requestData.subject,
			"Recover hello",
		);
		...
		assert.strictEqual(
			replyActionAfterRetry.requestData.subject,
			"Recover hello",
		);
...
void test("integration: Stripe authorization webhook verifies signature and writes a canonical event", async () => {
	const server = await buildServer();
	const { orgId, agentId } = await createOrgAndAgent(
		server,
		"Stripe Webhook Auth",
	);
	...
	try {
		...
		assert.strictEqual(
			storedEvents[0].data.authorization_id,
			"iauth_webhook_123",
		);
...
void test("integration: Stripe authorization webhook accepts string card IDs", async () => {
	...
	const { orgId, agentId } = await createOrgAndAgent(
		server,
		"Stripe Webhook Auth String Card",
	);
	...
	try {
		...
		assert.strictEqual(
			storedEvents[0].providerEventId,
			"evt_webhook_auth_string_123",
		);
		assert.strictEqual(
			storedEvents[0].data.authorization_id,
			"iauth_webhook_string_123",
		);
		assert.strictEqual(storedEvents[0].data.amount, 5000);
		assert.strictEqual(storedEvents[0].data.currency, "USD");
	} finally {
...
void test("integration: Stripe refund webhook verifies signature and preserves refund amounts", async () => {
	...
	try {
		...
		assert.strictEqual(
			storedEvents[0].providerEventId,
			"evt_webhook_refund_123",
		);
		assert.strictEqual(
			storedEvents[0].data.transaction_id,
			"ipi_webhook_refund_123",
		);
		assert.strictEqual(
			storedEvents[0].data.authorization_id,
			"iauth_webhook_refund_123",
		);
