...
import { EventBatchTooLargeError, MAX_INGEST_BATCH_SIZE } from '../src/domain/event-writer';
...
void test('integration: EventWriter idempotency and events query pagination work end-to-end', async () => {
  ...
  try {
    ...
    const providerWithBackfilledIdempotencyWrite = await server.eventWriter.writeEvent({
      orgId,
      agentId,
      provider: 'agentmail',
      providerEventId: 'provider_evt_1',
    ...
    assert.strictEqual(providerWithBackfilledIdempotencyWrite.wasCreated, false);
    assert.strictEqual(
    ...
    assert.strictEqual(idempotencyOnlyRetry.event.id, firstProviderWrite.event.id);
    assert.strictEqual(idempotencyOnlyRetry.event.providerEventId, 'provider_evt_1');
    assert.strictEqual(idempotencyOnlyRetry.event.idempotencyKey, 'idem_provider_1');
...
void test('integration: split dedupe key races converge to a single canonical row', async () => {
  ...
  try {
    ...
    for (let index = 0; index < 12; index += 1) {
      ...
      const [providerAndIdempotencyWrite, providerOnlyWrite] = await Promise.all([
        server.eventWriter.writeEvent({
          orgId,
          agentId,
          provider: 'agentmail',
      ...
      assert.strictEqual(providerAndIdempotencyWrite.event.id, matchedRows[0].id);
...
void test('integration: batch ingestion rejects oversized event arrays', async () => {
  ...
  try {
    ...
    const oversizedItems = Array.from({ length: MAX_INGEST_BATCH_SIZE + 1 }, (_, index) => {
      const indexText = String(index);
      return {
        orgId,
        agentId,
