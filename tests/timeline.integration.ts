  ...
  try {
    ...
    assert.ok(
      secondPagePayload.items.every(
        (item) => item.groupKey !== 'thread_email_1' && item.groupKey !== 'iauth_timeline_1',
      ),
    );
...
void test('integration: derived timeline preserves refund amounts in card activity summaries', async () => {
  ...
  try {
    ...
    assert.strictEqual(payload.items[0].latestEventType, EVENT_TYPES.PAYMENT_CARD_SETTLED);
    assert.strictEqual(payload.items[0].summary.authorizationId, 'iauth_timeline_refund_1');
    assert.strictEqual(payload.items[0].summary.transactionId, 'ipi_timeline_refund_1');
    assert.strictEqual(payload.items[0].summary.amount, -2500);
    assert.strictEqual(payload.items[0].summary.currency, 'USD');
    assert.strictEqual(payload.items[0].events[0].eventType, EVENT_TYPES.PAYMENT_CARD_SETTLED);
