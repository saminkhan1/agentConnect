...
function createLiveStripeClient() {
	assert.ok(
		liveStripeSecretKey,
		"Expected STRIPE_SECRET_KEY to be set for live smoke tests",
	);
...
async function createOrgAndAgent(
	...
	assert.strictEqual(
		createAgentResponse.statusCode,
		201,
		createAgentResponse.payload,
	);
...
liveSmokeTest(
	...
	async () => {
		...
		try {
			...
			assert.ok(
				storedResource,
				"Expected issued card resource to be persisted",
			);
			assert.strictEqual(storedResource.provider, "stripe");
			assert.strictEqual(
				storedResource.providerRef,
				payload.resource.providerRef,
			);
			...
			const liveCard = await stripe.issuing.cards.retrieve(
				payload.resource.providerRef,
			);
	...
	async () => {
		...
		try {
			...
			const authorizationResponse = await server.inject({
				method: "POST",
				url: "/webhooks/stripe",
				headers: buildStripeWebhookHeaders(
			...
			const refundObject = await stripe.testHelpers.issuing.transactions.refund(
				captureObject.id,
				{
					refund_amount: 100,
				},
			...
			assert.ok(
				authorizationEvent,
				"Expected live authorization webhook to be persisted",
			);
			...
			assert.strictEqual(
				...
				captureObject.currency.toUpperCase(),
			);
			...
			assert.strictEqual(
				refundData.currency,
				refundObject.currency.toUpperCase(),
			);
...
liveCardDetailsSmokeTest(
	...
	async () => {
		...
		try {
			...
			assert.strictEqual(
				payload.session.card_id,
				issuedCard.resource.providerRef,
			);
			assert.strictEqual(
				payload.session.stripe_api_version,
				STRIPE_API_VERSION,
			);
