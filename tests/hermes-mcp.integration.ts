...
function normalizeHermesToolName(serverName: string, toolName: string) {
  const normalize = (value: string) => value.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '');
...
void test(
  'integration: Hermes-style HTTP MCP bootstrap, scoped discovery, tool dispatch, and reconnect recovery',
  { concurrency: false },
  async () => {
    ...
    try {
      ...
      const adapterCalls: Array<{ action: string; payload: Record<string, unknown> }> = [];
      ...
      const anonymousClient = new Client({ name: 'hermes-anon', version: '0.0.0' });
      ...
      const serviceBootstrap = serviceKeyResult.structuredContent as { service_key: string };
      ...
      assert.ok(rootToolNames.includes('agentinfra.payments.create_card_details_session'));
      assert.strictEqual(
        new Set(rootToolNames.map((toolName) => normalizeHermesToolName('agentconnect', toolName)))
          .size,
      ...
      const serviceClient = new Client({ name: 'hermes-service', version: '0.0.0' });
      ...
      assert.ok(!serviceToolNames.includes('agentinfra.payments.create_card_details_session'));
      ...
      assert.ok(listedEvents.events.every((event) => event.eventType === 'email.sent'));
      assert.ok(
        listedEvents.events.some((event) => event.data.thread_id === 'thread_hermes'),
      ...
      assert.match(missingAgentResult.content[0].text, /NOT_FOUND: Agent not found/);

      const restoreResourceManager = installResourceManagerMock(requireServer(server), {
        provision: () => {
          return Promise.resolve({
        ...
        writeEvent: () => {
          ...
          return Promise.resolve({
            ...
            event: buildEventRecord({
              ...
              data: { card_id: fakeCardResource.providerRef ?? fakeCardResource.id },
            }),
          });
        },
      });
      ...
      assert.ok(afterRestart.agents.some((agent) => agent.id === createdAgent.agent.id));
    } finally {
