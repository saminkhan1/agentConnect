...
void test("integration: deprovision → state=deleted, not returned in GET", async () => {
	...
	try {
		...
		assert.strictEqual(provisionResponse.statusCode, 201);
		const resourceId = (
			JSON.parse(provisionResponse.payload) as { resource: { id: string } }
		).resource.id;
...
void test("integration: cross-org isolation: another org cannot deprovision first org resource", async () => {
	...
	try {
		...
		const agentAId = (
			JSON.parse(createAgentResponseA.payload) as { agent: { id: string } }
		).agent.id;
		...
		const agentBId = (
			JSON.parse(createAgentResponseB.payload) as { agent: { id: string } }
		).agent.id;
		...
		const storedResources = await db
			.select()
			.from(resources)
			.where(eq(resources.orgId, orgAId));
		assert.strictEqual(storedResources.length, 1);
		assert.strictEqual(storedResources[0].state, "active");
	} finally {
