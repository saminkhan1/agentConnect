...
async function buildAuthenticatedMcpSession(
  ...
  const { authorizationHeader, restore } = await installAuthApiKey(server, options);
...
void test('mapRestError maps REST statuses to MCP error codes and prefixes', () => {
  const cases = [
    { status: 400, message: 'bad input', code: ErrorCode.InvalidParams, prefix: 'bad input' },
    {
      status: 401,
      message: 'no key',
...
void test('readPackageVersion walks up from a compiled-like directory', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'agentconnect-version-'));
...
void test('buildMcpServer without auth registers only bootstrap tools', async () => {
  ...
  try {
    const mcp = buildMcpServer(server, { auth: null, authorizationHeader: null });
    const tools = Object.keys(
      (mcp as unknown as { _registeredTools: Record<string, unknown> })._registeredTools,
    );
    assert.ok(tools.includes('agentinfra.orgs.create'), 'orgs.create should be registered');
    assert.ok(
    ...
    assert.ok(!tools.includes('agentinfra.email.send'), 'email.send should NOT be registered');
    assert.ok(
...
void test('buildMcpServer with stdio auth fallback registers agent tools and honors per-call authorization', async () => {
  const server = await buildServer();
  const { authorizationHeader, restore } = await installAuthApiKey(server, { keyType: 'root' });
  ...
  try {
    ...
    assert.ok(registeredTools['agentinfra.agents.get'], 'agents.get should be registered');
    ...
    const parsed = JSON.parse(result.content[0].text) as { agent: { id: string } };
...
void test('buildMcpServer with root auth registers all tools', async () => {
  const server = await buildServer();
  const { session, restore } = await buildAuthenticatedMcpSession(server, { keyType: 'root' });
  try {
    ...
    assert.strictEqual(tools.length, expectedTools.length, 'Exactly 17 tools should be registered');
  } finally {
    restore();
    await server.close();
  }
...
void test('buildMcpServer with service auth hides root-only bootstrap tools', async () => {
  const server = await buildServer();
  const { session, restore } = await buildAuthenticatedMcpSession(server, { keyType: 'service' });
  try {
    const mcp = buildMcpServer(server, session);
    const tools = Object.keys(
      (mcp as unknown as { _registeredTools: Record<string, unknown> })._registeredTools,
...
void test('buildMcpServer with service auth plus stdio fallback hides root-only payment tools', async () => {
  ...
  const { session, restore } = await buildAuthenticatedMcpSession(server, { keyType: 'service' });
  try {
    const mcp = buildMcpServer(server, { ...session, allowToolAuthorizationFallback: true });
    const tools = Object.keys(
      (mcp as unknown as { _registeredTools: Record<string, unknown> })._registeredTools,
    );

    assert.ok(tools.includes('agentinfra.payments.issue_card'));
    assert.ok(!tools.includes('agentinfra.payments.create_card_details_session'));
  } finally {
    restore();
    await server.close();
  }
...
void test('agentinfra.payments.issue_card returns a pending result for in-progress replays', async () => {
  ...
  const originalRM = server.resourceManager;
  server.resourceManager = {
    provision: () => Promise.resolve({ resource: pendingResource, reusedExisting: true }),
  } as unknown as typeof server.resourceManager;
...
void test('agentinfra.payments.create_card_details_session keeps the ephemeral key out of text output', async () => {
  const server = await buildServer();
  const { session, restore } = await buildAuthenticatedMcpSession(server, { keyType: 'root' });
  const agent = buildAgentRecord();
  const restoreAgents = installAgentsDalMock({ findById: () => Promise.resolve(agent) });
  ...
  try {
    ...
    const detailsTool = registeredTools['agentinfra.payments.create_card_details_session'];
    assert.ok(detailsTool, 'create_card_details_session tool should be registered');
    ...
    const result = (await detailsTool.handler(
      ...
      content: Array<{ type: string; text: string }>;
      structuredContent: { session: { ephemeral_key_secret: string; card_id: string } };
    };

    const textContent = result.content[0].text;
    assert.ok(textContent.includes('"card_id": "ic_test123"'));
    assert.ok(!textContent.includes('ephkey_test_secret'));
    assert.strictEqual(result.structuredContent.session.card_id, 'ic_test123');
    assert.strictEqual(result.structuredContent.session.ephemeral_key_secret, 'ephkey_test_secret');
  } finally {
    server.stripeAdapter = originalStripe;
    restore();
    restoreAgents();
...
void test('agentinfra.orgs.create returns root_key in structuredContent', async () => {
  ...
  const originalCreate = server.systemDal.createOrgWithApiKey.bind(server.systemDal);
  ...
  try {
    const mcp = buildMcpServer(server, { auth: null, authorizationHeader: null }); // no auth — bootstrap only
    const registeredTools = (
      mcp as unknown as {
        _registeredTools: Record<string, { handler: (...args: unknown[]) => Promise<unknown> }>;
      }
    ...
    const result = (await createOrgTool.handler({ name: 'Test Org' }, {} as never)) as {
      content: Array<{ type: string; text: string }>;
...
void test('GET /agents/:id/messages/:messageId returns the provider message payload', async () => {
  const server = await buildServer();
  const { authorizationHeader, restore } = await installAuthApiKey(server, { keyType: 'service' });
  const agent = buildAgentRecord();
