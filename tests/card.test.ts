...
import type { ParsedWebhookEvent, ProviderAdapter } from '../src/adapters/provider-adapter';
...
void test('POST /agents/:id/actions/issue_card rejects missing explicit Stripe cardholder fields', async () => {
  ...
  try {
    ...
    assert.match(response.json<{ message: string }>().message, /cardholder_name/i);
  } finally {
    restore();
    await server.close();
  }
...
void test('POST /agents/:id/actions/issue_card rejects mutually exclusive Stripe spending controls', async () => {
  ...
  try {
    ...
    assert.strictEqual(response.statusCode, 400);
    assert.match(response.json<{ message: string }>().message, /mutually exclusive/i);
  } finally {
    restore();
    await server.close();
  }
...
void test('POST /agents/:id/actions/create_card_details_session returns 404 for a card owned by another agent', async () => {
  ...
  try {
    ...
    assert.deepStrictEqual(response.json(), { message: 'Card resource not found' });
  } finally {
    restore();
    restoreAgents();
    restoreResources();
...
void test('issueCardBodySchema accepts Stripe spending-limit categories and intervals', () => {
  ...
  assert.deepStrictEqual(parsed.blocked_categories, ['art_dealers_and_galleries']);
  ...
  (
    adapter as unknown as {
      stripe: {
        issuing: {
          cardholders: {
            create: (params: Stripe.Issuing.CardholderCreateParams) => Promise<{ id: string }>;
            update: (id: string, params: { status: 'inactive' }) => Promise<void>;
          };
          cards: {
            create: (
              params: Stripe.Issuing.CardCreateParams,
            ) => Promise<Pick<Stripe.Issuing.Card, 'id' | 'last4' | 'exp_month' | 'exp_year'>>;
          };
        };
      };
    }
...
void test('StripeAdapter.provision deactivates the cardholder when card creation fails', async () => {
  ...
  await assert.rejects(adapter.provision('agt_123', buildIssueCardPayload()), /card create failed/);
...
void test('StripeAdapter.createCardDetailsSession uses the nonce and pinned API version', async () => {
  ...
  const session = await adapter.createCardDetailsSession(buildCardResourceRecord(), 'nonce_123');

  assert.deepStrictEqual(ephemeralKeyParams, {
    issuing_card: 'ic_test123',
    nonce: 'nonce_123',
  });
  assert.deepStrictEqual(ephemeralKeyOptions, { apiVersion: STRIPE_API_VERSION });
...
void test('ResourceManager.provision deprovisions a Stripe card when DB activation fails', async () => {
  ...
  const fakeDal = {
    resources: {
      ...
      updateById: () => Promise.reject(new Error('forced resource activation failure')),
    },
  } as unknown as DalFactory;

  const manager = new ResourceManager(new Map([['stripe', fakeAdapter]]));
  await assert.rejects(
    manager.provision(fakeDal, 'agt_123', 'card', 'stripe', buildIssueCardPayload()),
      ...
      Promise.resolve([
        {
          ...
          occurredAt: FIXED_TIMESTAMP,
          data: { authorization_id: 'iauth_missing_123', amount: 5000, currency: 'USD' },
      ...
      Promise.resolve([
        {
          ...
          eventType: EVENT_TYPES.PAYMENT_CARD_AUTHORIZED,
          occurredAt: FIXED_TIMESTAMP,
          data: { authorization_id: 'iauth_write_failure_123', amount: 5000, currency: 'USD' },
        } satisfies ParsedWebhookEvent,
      ]),
  });
  const restoreWriter = installEventWriterMock(server, {
