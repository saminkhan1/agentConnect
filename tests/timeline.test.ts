...
void test('buildTimelineItem creates an email_thread summary with de-duped recipients', () => {
  ...
  const item = buildTimelineItem('email_thread', 'thread_1', [olderEvent, newerEvent]);
...
void test('buildTimelineItem creates a card_activity summary from auth plus settle events', () => {
  ...
  const item = buildTimelineItem('card_activity', 'iauth_1', [authorizationEvent, settlementEvent]);
...
void test('buildTimelineItem creates a singleton event summary', () => {
  ...
  assert.strictEqual(item.summary.value.eventType, EVENT_TYPES.PAYMENT_CARD_ISSUED);
...
void test('GET /agents/:id/timeline returns mixed items and an opaque nextCursor', async () => {
  ...
  try {
    ...
    const payload = listAgentTimelineResponseSchema.parse(JSON.parse(response.payload));
    ...
    assert.strictEqual(capturedCalls[0].options.until?.toISOString(), '2026-03-05T00:00:00.000Z');
  } finally {
    restore();
    restoreAgentsDal();
    restoreEventsDal();
