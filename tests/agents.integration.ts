...
void test('integration: agent CRUD uses real DB and enforces org isolation', async () => {
  ...
  try {
    ...
    const createdAgent = (createPayload as { agent: { id: string; orgId: string } }).agent;
    ...
    const listAfterArchivePayload = JSON.parse(listAfterArchiveResponse.payload) as {
    ...
    const listWithArchivedPayload = JSON.parse(listWithArchivedResponse.payload) as {
      agents: Array<{ id: string; isArchived: boolean }>;
    };
    ...
    const dbLookup = await db
      .select()
      .from(agents)
      .where(and(eq(agents.orgId, primaryOrgId), eq(agents.id, createdAgent.id)))
      .limit(1);
