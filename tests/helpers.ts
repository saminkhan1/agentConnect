...
import type { buildServer } from "../src/api/server";
...
export function buildCardResourceRecord(
	overrides?: Partial<ResourceRecord>,
): ResourceRecord {
	return {
		id: "res_card_123",
...
export function installAdvisoryLockMock(
	server: Awaited<ReturnType<typeof buildServer>>,
) {
	const original = server.withAdvisoryLock.bind(server);
	const pendingLocks = new Map<
		string,
		{ tail: Promise<void>; waiters: number }
	>();

	server.withAdvisoryLock = async <T>(
		lockKey: string,
		callback: () => Promise<T>,
	) => {
		const state = pendingLocks.get(lockKey) ?? {
...
export function installEventWriterMock(
	...
	server.webhookProcessor = new (
		server.webhookProcessor.constructor as new (
			ew: EventWriter,
		) => typeof server.webhookProcessor
	)(server.eventWriter);
	return () => {
...
export function installOutboundActionsDalMock(methods: {
	...
	createReady?: (
		input: OutboundActionCreateReadyInput,
	) => Promise<OutboundActionRecord>;
	transitionState?: (
		id: string,
		fromState: OutboundActionState,
		toState: OutboundActionState,
	...
	return {
		...
		methods: {
			findByIdempotencyKey: async (
				...
				await options?.onFindByIdempotencyKey?.(key, action);
				return currentAction;
			},
			...
			transitionState: async (
				...
				if (currentAction.state !== fromState) {
					return null;
				}

				await options?.onTransitionState?.(
					currentAction,
					fromState,
					toState,
					updates,
				...
				currentAction = {
					...
					providerResult:
						updates?.providerResult ?? currentAction.providerResult,
