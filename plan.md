...
| Commit                                                     | What                                                                                                                              |
| ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `feat: AgentMailAdapter provision + deprovision`           | Create inbox → store `provider_ref` (email address/inbox id).                                                                     |
| `feat: POST /agents/:id/actions/send_email`                | Lookup email resource → `enforceEmailPolicy` → adapter send → emit `email.sent` event (idempotency via action `idempotency_key`). |
| `feat: inbound webhook endpoint`                           | `/webhooks/agentmail` - Verify signature (Svix headers), enqueue job, fast ACK.                                                   |
| `feat: webhook job processor (email)`                      | Parse provider payload → map to `email.received`, `email.delivered`, `email.bounced` → write events idempotently.                 |
| `feat: provider client wrapper`                            | Timeouts, retries with jitter for outbound API calls; circuit-breaker hooks (lightweight).                                        |
| `test: email send + webhook ingestion e2e (mock provider)` | Mock AgentMail API + signed webhook payloads; assert events in DB.                                                                |
| `docs: provider integration contract`                      | Document required webhook fields you depend on (thread id, message id, timestamps).                                               |
...
| Commit                                                        | What                                                                                                                                                                   |
| ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `feat: CardAdapter provision (issue card resource)`           | Provision a card as a `resource(type=card)` with `provider_ref` = card id/token. Store config: `max_amount`, `currency`, `ttl_minutes`, optional `merchant_lock`.      |
| `feat: POST /agents/:id/actions/issue_card`                   | Creates card; returns PAN/exp/CVV once; redacts logs; stores only masked + token refs. Emits `payment.card.issued` event.                                              |
| `feat: inbound webhook endpoint`                              | `/webhooks/card_issuer` - Verify signature, enqueue job, fast ACK.                                                                                                     |
| `feat: webhook job processor (card)`                          | Map provider events to canonical: `payment.card.authorized`, `payment.card.declined`, `payment.card.settled`. Use `(org_id, provider, provider_event_id)` idempotency. |
| `feat: config enforcement for issuance`                       | Enforce max spend / merchant lock at issuance time (MVP). (Defer realtime auth decisioning.)                                                                           |
| `test: card issuance + webhook ingestion e2e (mock provider)` | Ensure no sensitive fields stored/logged; events written correctly.                                                                                                    |
