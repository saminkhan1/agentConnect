...
type IssueCardResponse = { resource: unknown; card: IssuedCardMetadata; event: unknown };
...
function isCardDetailsSession(value: unknown): value is CardDetailsSession {
  return (
    ...
    typeof (value as { ephemeral_key_secret?: unknown }).ephemeral_key_secret === 'string' &&
    typeof (value as { expires_at?: unknown }).expires_at === 'number' &&
    typeof (value as { livemode?: unknown }).livemode === 'boolean' &&
    typeof (value as { stripe_api_version?: unknown }).stripe_api_version === 'string'
  );
}
...
function buildPendingIssueCardResponse(message: string): PendingIssueCardResponse {
  return {
    status: 'pending',
    message,
  };
  ...
  server.registerTool(
    'agentinfra.payments.issue_card',
    {
      ...
      inputSchema: withOptionalAuthorizationSchema(session, {
        ...
        cardholder_name: z.string().min(1).max(24).describe('Name printed on the card'),
        billing_address: z.object({
          ...
          country: stripeIssuingMerchantCountrySchema.describe('ISO 3166-1 alpha-2 country code'),
        }),
    ...
    async ({
      ...
      if (allowed_categories !== undefined) body['allowed_categories'] = allowed_categories;
      if (blocked_categories !== undefined) body['blocked_categories'] = blocked_categories;
      ...
      if (!isIssueCardResponse(data)) {
        const pending = buildPendingIssueCardResponse(
          statusCode === 202
            ? (extractPendingMessage(data) ?? 'Card issuance is still in progress')
      ...
      return {
        ...
        structuredContent: { resource: data.resource, card: data.card, event: data.event },
      };
    },
  );
  ...
  if (
    ...
    server.registerTool(
      ...
      async ({ agent_id, resource_id, nonce, authorization }) => {
        ...
        const safeSession = {
          resource_id: data.session.resource_id,
          card_id: data.session.card_id,
          expires_at: data.session.expires_at,
          livemode: data.session.livemode,
          stripe_api_version: data.session.stripe_api_version,
        };
