...
export function mapRestError(status: number, message: string): McpError {
  ...
  if (status === 422) {
    return new McpError(ErrorCode.InvalidParams, `UNPROCESSABLE_ENTITY: ${message}`);
  }
  if (status === 401) return new McpError(ErrorCode.InvalidRequest, `UNAUTHENTICATED: ${message}`);
  if (status === 403) return new McpError(ErrorCode.InvalidRequest, `FORBIDDEN: ${message}`);
  if (status === 404) return new McpError(ErrorCode.InvalidRequest, `NOT_FOUND: ${message}`);
  if (status === 409) return new McpError(ErrorCode.InvalidRequest, `CONFLICT: ${message}`);
  if (status === 429) return new McpError(ErrorCode.InternalError, `RATE_LIMITED: ${message}`);
  if (status === 503) return new McpError(ErrorCode.InternalError, `UNAVAILABLE: ${message}`);
  return new McpError(ErrorCode.InternalError, `INTERNAL: ${message}`);
}

export function buildQueryString(fields: Record<string, string | number | undefined>) {
