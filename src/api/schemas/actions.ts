...
export const replyEmailResponseSchema = z.object({ event: eventResponseSchema });
...
export const issueCardBodySchema = z
  .object({
    ...
    blocked_merchant_countries: z.array(stripeIssuingMerchantCountrySchema).optional(),
    idempotency_key: z.string().min(1).optional(),
  })
  .superRefine((value, context) => {
    if (value.allowed_categories && value.blocked_categories) {
      context.addIssue({
        code: 'custom',
        message: 'allowed_categories and blocked_categories are mutually exclusive',
    ...
    if (value.allowed_merchant_countries && value.blocked_merchant_countries) {
      context.addIssue({
        code: 'custom',
        message: 'allowed_merchant_countries and blocked_merchant_countries are mutually exclusive',
