...
>(args: {
	...
	const [agent, emailResource] = await Promise.all([
		...
		dal.resources.findActiveByAgentIdAndType(
			agentId,
			"email_inbox",
			"agentmail",
		),
	...
	const executeDirectAction = async () => {
		...
		try {
			...
			if (
				replyFromAgentMailError(reply, error, config.dispatchFailureMessage)
			) {
				return reply;
			}
	...
	const executeIdempotentAction = async () => {
		...
		let actionRow = await dal.outboundActions.findByIdempotencyKey(
		...
		if (actionRow) {
			...
			const conflict = isOutboundActionConflict(
				actionRow,
				config.actionType,
				requestHash,
			);
		...
		const dispatchRequestData = config.parseStoredRequestData(
			durableAction.requestData,
		);
		...
		if (durableAction.state === "rejected") {
			...
			if (!cachedError) {
				return reply
					.code(500)
					.send({ message: MSG_OUTBOUND_ACTION_INCOMPLETE });
			}
		...
		if (
			durableAction.state === "completed" ||
			durableAction.state === "provider_succeeded"
		) {
		...
		if (
			durableAction.state === "dispatching" ||
			durableAction.state === "ambiguous"
		) {
		...
		if (!dispatchingAction) {
			return reply
				.code(500)
				.send({ message: "Unable to update outbound action state" });
		}
		...
		try {
			...
			if (definitiveError) {
				await dal.outboundActions
					.transitionState(actionRow.id, "dispatching", "rejected", {
			...
			const retryableError = serializeRetryableAgentMailError(
				error,
				config.dispatchFailureMessage,
			);
			if (retryableError) {
				await dal.outboundActions
					.transitionState(actionRow.id, "dispatching", "ready", {
			...
			await dal.outboundActions
				.transitionState(actionRow.id, "dispatching", "ambiguous", {
		...
		if (!providerSucceededAction) {
			return reply
				.code(500)
				.send({ message: "Unable to persist outbound action result" });
		}
		...
		await dal.outboundActions
			.transitionState(actionRow.id, "provider_succeeded", "completed", {
				eventId: event.id,
			})
			.catch((err: unknown) => {
				request.log.warn({ err }, "Failed to transition action state");
...
function isSentEmailEventForResource(
	event: EventRecord,
	agentId: string,
	resourceId: string,
) {
...
async function resolveCompletedActionEvent(
	...
	const fallbackEvent = await dal.events.findByIdempotencyKey(
...
async function recoverFromStoredProviderResult<
	TRequestData extends Record<string, unknown>,
>(args: {
	request: FastifyRequest;
	reply: FastifyReply;
	...
	const resolvedEvent = await resolveCompletedActionEvent(dal, action, {
		isExpectedEvent: (event) =>
			isSentEmailEventForResource(event, agentId, resource.id),
		recover: async () => {
			const { event: recoveredEvent } =
				await request.server.eventWriter.writeEvent({
					orgId,
					agentId,
			...
			await dal.outboundActions
				.transitionState(action.id, "provider_succeeded", "completed", {
	...
	if (action.state !== "completed") {
		await dal.outboundActions
			.transitionState(
				action.id,
				action.state as OutboundActionState,
				"completed",
				{ eventId: event.id },
			)
			.catch((err: unknown) => {
