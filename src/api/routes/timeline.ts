...
function serializeTimelineItem(item: TimelineItem) {
  ...
  if (item.summary.kind === 'email_thread') {
    return { ...baseItem, kind: 'email_thread' as const, summary: item.summary.value };
  }

  if (item.summary.kind === 'card_activity') {
    return { ...baseItem, kind: 'card_activity' as const, summary: item.summary.value };
  }
...
const timelineRoutes: FastifyPluginCallbackZod = (server, _opts, done) => {
  server.get(
    ...
    async (request, reply) => {
      ...
      const nextCursor = hasMore ? encodeTimelineCursor(pagedItems[pagedItems.length - 1]) : null;
