...
const resourceRoutes: FastifyPluginCallbackZod = (server, _opts, done) => {
  server.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        return await reply.code(201).send({ resource: serializeResource(resource) });
      } catch (err) {
  ...
  server.get(
    ...
    async (request, reply) => {
      ...
      const agentResources = await dal.resources.findByAgentId(request.params.id);
      return reply.code(200).send({ resources: agentResources.map(serializeResource) });
    },
  );
  ...
  server.delete(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        return await reply.code(200).send({ resource: serializeResource(resource) });
      } catch (err) {
        if (err instanceof AppError) {
          return reply.code(404).send({ message: err.message });
        }
