...
const webhookSubscriptionRoutes: FastifyPluginCallbackZod = (server, _opts, done) => {
  server.post(
    ...
    async (request, reply) => {
      ...
      const { subscription, signingSecret } =
        await server.outboundWebhookService.createSubscription(dal, request.body);
      ...
      return reply.code(201).send({
        subscription: {
          ...
          staticHeaders: subscription.staticHeaders,
