...
function getExpandableId(value: string | { id: string } | null | undefined): string | undefined {
...
function buildSpendingControls(
      ...
      ? config['spending_limits'].flatMap((entry) => {
          ...
          const categories =
            Array.isArray(candidate['categories']) && candidate['categories'].length > 0
              ? (candidate[
                  'categories'
                ] as Stripe.Issuing.CardCreateParams.SpendingControls.SpendingLimit.Category[])
              : undefined;
          ...
          return [
            {
              ...
              interval: candidate[
                'interval'
              ] as Stripe.Issuing.CardCreateParams.SpendingControls.SpendingLimit.Interval,
            },
          ];
        })
      : undefined;
  const allowed_categories =
    Array.isArray(config['allowed_categories']) && config['allowed_categories'].length > 0
      ? (config[
          'allowed_categories'
        ] as Stripe.Issuing.CardCreateParams.SpendingControls.AllowedCategory[])
      : undefined;
  const blocked_categories =
    Array.isArray(config['blocked_categories']) && config['blocked_categories'].length > 0
      ? (config[
          'blocked_categories'
        ] as Stripe.Issuing.CardCreateParams.SpendingControls.BlockedCategory[])
      : undefined;
  ...
  return Object.keys(spendingControls).length > 0 ? spendingControls : undefined;
}
...
function buildTransactionEvent(event: Stripe.IssuingTransactionCreatedEvent): ParsedWebhookEvent {
...
export class StripeAdapter implements ProviderAdapter {
  ...
  private constructWebhookEvent(rawBody: Buffer, headers: Record<string, string>): Stripe.Event {
