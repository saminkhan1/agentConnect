...
import { type EventType, eventTypeSchema, eventTypeValues } from "./events";
...
export const webhookSubscriptionStatusSchema = z.enum(
	webhookSubscriptionStatusEnum.enumValues,
);
export const webhookDeliveryStatusSchema = z.enum(
	webhookDeliveryStatusEnum.enumValues,
);
...
export const openClawHookAgentDeliveryConfigSchema = z
	.object({
		...
		session_key_prefix: nonEmptyStringSchema
			.regex(/^[A-Za-z0-9:_-]+$/)
			.optional(),
...
export const outboundWebhookCreateInputSchema = z
	.object({
		...
		delivery_mode: webhookSubscriptionDeliveryModeSchema
			.optional()
...
export type WebhookSubscriptionDeliveryMode = z.infer<
	typeof webhookSubscriptionDeliveryModeSchema
>;
export type WebhookSubscriptionStatus = z.infer<
	typeof webhookSubscriptionStatusSchema
>;
export type WebhookDeliveryStatus = z.infer<typeof webhookDeliveryStatusSchema>;
export type OutboundWebhookStaticHeaders = z.infer<
	typeof outboundWebhookStaticHeadersSchema
>;
export type OpenClawHookAgentDeliveryConfig = z.infer<
	typeof openClawHookAgentDeliveryConfigSchema
>;
export type OpenClawHookWakeDeliveryConfig = z.infer<
	typeof openClawHookWakeDeliveryConfigSchema
>;
export type CanonicalWebhookDeliveryConfig = z.infer<
	typeof canonicalWebhookDeliveryConfigSchema
>;
export type OutboundWebhookCreateInput = z.infer<
	typeof outboundWebhookCreateInputSchema
>;
...
function normalizeEventTypes(eventTypes?: EventType[]) {
	const normalizedEventTypes: EventType[] = eventTypes
		? [...eventTypes]
		: [...eventTypeValues];
...
function isAllowlistedHost(hostname: string, allowlistedHosts: string[]) {
	return allowlistedHosts.some((pattern) =>
		hostMatchesAllowedPattern(hostname, pattern),
	);
}
...
export async function validateOutboundWebhookUrl(
	...
	if (
		deliveryMode === "openclaw_hook_agent" &&
		!normalizedPathname.endsWith("/agent")
	) {
		throw new AppError(
			"OUTBOUND_WEBHOOK_URL_INVALID",
			400,
	...
	if (
		deliveryMode === "openclaw_hook_wake" &&
		!normalizedPathname.endsWith("/wake")
	) {
		throw new AppError(
			"OUTBOUND_WEBHOOK_URL_INVALID",
			400,
...
function parseStoredStaticHeaders(value: unknown) {
	return normalizeStaticHeaders(
		outboundWebhookStaticHeadersSchema.parse(value),
	);
}
...
export function serializeOutboundCanonicalEvent(
	event: EventRecord,
): OutboundCanonicalEvent {
	return {
		id: event.id,
		org_id: event.orgId,
...
function buildOpenClawAgentPayload(
	subscription: Pick<
		...
		"id" | "deliveryMode" | "deliveryConfig"
	>,
	event: EventRecord,
) {
	const config = openClawHookAgentDeliveryConfigSchema.parse(
		subscription.deliveryConfig,
	);
	const message = JSON.stringify(
		buildOpenClawMessageEnvelope(subscription, event),
	);
	...
	return {
		...
		...(config.session_key_prefix
			? { sessionKey: `${config.session_key_prefix}${event.id}` }
			: {}),
...
function buildOpenClawWakePayload(
	...
	const config = openClawHookWakeDeliveryConfigSchema.parse(
		subscription.deliveryConfig,
	);
...
export function buildDeliveryAttemptRequest(
	...
	const signature = buildOutboundWebhookSignature(
...
/**
 * Read at most MAX_RESPONSE_BODY_BYTES from a fetch Response to prevent OOM
 * from malicious endpoints returning unbounded payloads.
 */
const MAX_RESPONSE_BODY_BYTES = 64 * 1024; // 64 KB — more than enough for any webhook response
	...
	try {
		for (;;) {
			...
			if (totalBytes > MAX_RESPONSE_BODY_BYTES) {
				chunks.push(
					value.slice(0, value.length - (totalBytes - MAX_RESPONSE_BODY_BYTES)),
				);
				break;
			}
			chunks.push(value);
...
export function getWebhookRetryDelayMs(
	attemptCount: number,
	random = Math.random,
) {
	const baseDelay =
		OUTBOUND_WEBHOOK_BASE_RETRY_DELAY_MS * 2 ** Math.max(0, attemptCount - 1);
...
export class OutboundWebhookService {
	...
	async createSubscription(
		dal: CreateWebhookSubscriptionDal,
		...
		const deliveryConfig = parseDeliveryConfig(
			deliveryMode,
			parsedInput.delivery_config,
		);
		const url = await validateOutboundWebhookUrl(
			parsedInput.url,
			deliveryMode,
			this.options,
		);
	...
	async listDeliveries(
		...
		const subscription =
			await dal.webhookSubscriptions.findById(subscriptionId);
		if (!subscription) {
			throw new AppError(
...
export class OutboundWebhookWorker {
      ...
      `);
		...
		for (const delivery of claimedDeliveries) {
			try {
				...
				// Log but do not stop processing remaining deliveries in the batch.
				// The lock on this delivery will expire after OUTBOUND_WEBHOOK_LOCK_TTL_MS.
				console.error(
					`Unhandled error processing webhook delivery ${delivery.id}:`,
	...
	private getRequestTimeoutMs() {
		return (
			...
			OUTBOUND_WEBHOOK_DEFAULT_REQUEST_TIMEOUT_MS
		);
	}
	...
	private async processDelivery(delivery: WebhookDeliveryRecord) {
		...
		const request = buildDeliveryAttemptRequest(
			row.subscription,
			row.delivery,
			row.event,
		);
		...
		try {
			...
			await assertSafeHostname(
				deliveryUrl.hostname,
				this.options.allowlistedHosts ?? [],
		...
		const nextAttemptAt = shouldRetry
			? new Date(
					updatedAt.getTime() + Math.max(retryDelayMs, retryAfterDelayMs ?? 0),
				)
			: updatedAt;
		...
		await db
			...
			.set({
				...
				lastResponseStatusCode:
					outcome.kind === "response" ? outcome.statusCode : null,
				lastResponseBody:
					outcome.kind === "response" ? outcome.responseBody : null,
				lastRequestHeaders: requestHeaders,
				lastPayload: request.payload,
				lastError:
