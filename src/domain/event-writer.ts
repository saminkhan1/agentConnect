...
import { type EventType, eventTypeSchema, validateEventData } from "./events";
...
function extractPgErrorMetadata(error: unknown): {
	code?: string;
	constraint?: string;
} {
...
function isRetryableWriteError(error: unknown): boolean {
	...
	return (
		...
		DEDUPE_CONSTRAINT_NAMES.has(constraint)
	);
}
...
export class EventWriter {
	...
	private async withWriteRetries<T>(operation: () => Promise<T>): Promise<T> {
		for (let attempt = 1; attempt <= MAX_WRITE_RETRY_ATTEMPTS; attempt += 1) {
			try {
				...
				if (
					attempt === MAX_WRITE_RETRY_ATTEMPTS ||
	...
	private async writeWithExecutor(
		...
		const existingEvent =
			duplicateMatches.byProvider ?? duplicateMatches.byIdempotency;
		if (!existingEvent) {
			throw new EventDedupeConflictError(
				"Insert was skipped without a matching dedupe key",
			);
		}

		const mergedEvent = await this.mergeMissingDedupeKeys(
			dbExecutor,
			existingEvent,
			parsedInput,
		);
	...
	private async acquireDedupeLocks(
		...
		if (isPresent(input.providerEventId)) {
			lockKeys.push(
				`events:${input.orgId}:provider:${input.provider}:${input.providerEventId}`,
			);
		}
		if (isPresent(input.idempotencyKey)) {
			lockKeys.push(
				`events:${input.orgId}:idempotency:${input.idempotencyKey}`,
			);
		}
	...
	private pickCanonicalEvent(
		left: EventRecord,
		right: EventRecord,
	): EventRecord {
		const ingestedAtDifference =
	...
	private async reconcileDuplicateMatches(
		...
		const lockedRows = await dbExecutor
			...
			.where(
				and(
					eq(events.orgId, input.orgId),
					or(
						eq(events.id, providerMatch.id),
						eq(events.id, idempotencyMatch.id),
					),
				),
			)
			.for("update");
		...
		if (!lockedProvider || !lockedIdempotency) {
			...
			if (!remainingEvent) {
				throw new EventDedupeConflictError(
					"Unable to reconcile conflicting event dedupe keys",
				);
			}
		...
		const canonicalEvent = this.pickCanonicalEvent(
			lockedProvider,
			lockedIdempotency,
		);
		const duplicateEvent =
			canonicalEvent.id === lockedProvider.id
				? lockedIdempotency
				: lockedProvider;
		...
		await dbExecutor
			...
			.where(
				and(
					eq(events.orgId, duplicateEvent.orgId),
					eq(events.id, duplicateEvent.id),
				),
			);

		const updates: { providerEventId?: string; idempotencyKey?: string } = {};
		if (
			canonicalEvent.providerEventId === null &&
			resolvedProviderEventId !== null
		) {
			updates.providerEventId = resolvedProviderEventId;
		}
		if (
			canonicalEvent.idempotencyKey === null &&
		...
		const updatedRows = await dbExecutor
			...
			.where(
				and(
					eq(events.orgId, canonicalEvent.orgId),
					eq(events.id, canonicalEvent.id),
				),
			)
			.returning();
		if (updatedRows.length === 0) {
			throw new EventDedupeConflictError(
				"Failed to reconcile conflicting event dedupe keys",
			);
		}
	...
	private async mergeMissingDedupeKeys(
		...
		const lockedRows = await dbExecutor
			...
			.where(
				and(
					eq(events.orgId, existingEvent.orgId),
					eq(events.id, existingEvent.id),
				),
			)
			.for("update")
			.limit(1);
		...
		const updates: { providerEventId?: string; idempotencyKey?: string } = {};
		if (
			lockedEvent.providerEventId === null &&
			isPresent(input.providerEventId)
		) {
			updates.providerEventId = input.providerEventId;
		}

		if (
			lockedEvent.idempotencyKey === null &&
			isPresent(input.idempotencyKey)
		) {
		...
		const updatedRows = await dbExecutor
			...
			.set(updates)
			.where(
				and(eq(events.orgId, lockedEvent.orgId), eq(events.id, lockedEvent.id)),
			)
			.returning();
		if (updatedRows.length === 0) {
			throw new EventDedupeConflictError(
				"Failed to merge dedupe keys into canonical event",
			);
		}
