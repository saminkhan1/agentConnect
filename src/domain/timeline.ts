...
export const timelineItemKindValues = [
	"email_thread",
	"card_activity",
	"event",
] as const;
...
const timelineItemIdPayloadSchema = z.tuple([
...
function buildEmailThreadSummary(
	groupKey: string,
...
function buildCardActivitySummary(
	sortedEvents: EventRecord[],
): TimelineSummary {
...
export function encodeTimelineItemId(
	kind: TimelineItemKind,
	groupKey: string,
): string {
...
export function encodeTimelineCursor(
	item: Pick<TimelineItem, "id" | "occurredAt">,
): string {
	return Buffer.from(
		JSON.stringify({
