...
import { withRetry } from "../adapters/provider-client";
...
export class ResourceManager {
	...
	async provision(
		...
		const existing = options?.resourceId
			? await dal.resources.findById(id)
			: null;
		if (existing) {
			...
			if (!reset) {
				throw new AppError(
					"INTERNAL",
					500,
					"Failed to reset idempotent resource state",
				);
			}
		...
		try {
			result = await withRetry(() => adapter.provision(agentId, config));
		} catch (err) {
		...
		if (!parsedConfig.success) {
			await cleanupProvisionedResource(
				adapter,
		...
		if (!updated) {
			...
			throw new AppError(
				"INTERNAL",
				500,
				"Resource update failed unexpectedly",
			);
		}
	...
	async deprovision(
		...
		await withRetry(() => adapter.deprovision(resource));

		const updated = await dal.resources.updateById(resourceId, {
			state: "deleted",
		});
		if (!updated)
			throw new AppError(
