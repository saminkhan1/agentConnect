...
export class EventDal {
	...
	async listByAgent(
		...
		if (options.cursor) {
			const cursorCondition = or(
				...
				and(
					eq(events.occurredAt, options.cursor.occurredAt),
					lt(events.id, options.cursor.id),
				),
			);
    ...
    `);
		...
		const selectedGroups = sql.join(
			groupedRows.map(
				(row) =>
					sql`(${row.itemId}, ${row.itemKind}, ${row.groupKey}, ${row.occurredAt})`,
			),
			sql`, `,
		);
...
export class OutboundActionDal {
	...
	async findByIdempotencyKey(
		...
		if (action) {
			conditions.push(eq(outboundActions.action, action));
		}
		const result = await db
			.select()
	...
	async updateById(
		...
		const result = await db
			...
			.where(
				and(eq(outboundActions.orgId, this.orgId), eq(outboundActions.id, id)),
			)
			.returning();
		return result[0] ?? null;
	}
	...
	async transitionState(
		...
		const result = await db
			...
			.where(
				and(
					eq(outboundActions.orgId, this.orgId),
					eq(outboundActions.id, id),
					eq(outboundActions.state, fromState),
				),
			)
			.returning();
		return result[0] ?? null;
...
export const systemDal = {
	...
	async createOrgWithApiKey(data: {
		org: NewOrg;
		apiKey: Omit<NewApiKey, "orgId">;
	}): Promise<{
		org: OrgRecord;
		apiKey: ApiKeyRecord;
	}> {
