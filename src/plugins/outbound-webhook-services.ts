...
const outboundWebhookServicesPlugin: FastifyPluginCallback = (server, _opts, done) => {
  const config = getServerConfig(process.env);

  server.decorate(
    'outboundWebhookService',
    new OutboundWebhookService({
      allowlistedHosts: parseOutboundWebhookAllowedHosts(config.OUTBOUND_WEBHOOK_ALLOWED_HOSTS),
